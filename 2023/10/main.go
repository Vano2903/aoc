...
func solve2(input [][]string) {
	...
	//0 => right, 1 => up, 2 => left, 3 => down
	currentX, currentY := start[0], start[1]
	obstacles := make(map[int]map[int]bool, 0)
	obstacles[currentX] = make(map[int]bool, 0)
	obstacles[currentX][currentY] = true
	all := make(map[int]map[int]bool, 0)
	all[currentX] = make(map[int]bool, 0)
	all[currentX][currentY] = true
	switch kind {
	case "J", "L", "|":
		direction = directionUp
		// if kind == "J" || kind == "|" {
	...
	// coordSum := 0
	pointCounter := 0
	for x := 0; x < len(pipes); x++ {
		for y := 0; y < len(pipes[x]); y++ {
			if pipes[x][y] == "." {
				c := 0
				//ray casting
				if obstacles[x] != nil {
					for yObstacle := range obstacles[x] {
						if yObstacle < y {
							continue
						}
						c++
	...
	// 			c := 0
	// 			//ray casting
	// 			if coordinates[x] != nil {
	// 				for yObstacle := range coordinates[x] {
	// 					// fmt.Println("k", k)
	// 					if yObstacle < y {
	// 						continue
	// 					}
	// 					fmt.Println("x", x, "y", y, "yObstacle", yObstacle)
	// 					c++
	// 				}
...
func floodfill(currentX, currentY int, matrix [][]string, visited map[int]map[int]bool) {
	...
	if currentY > 0 {
		if matrix[currentX][currentY-1] == "-" || matrix[currentX][currentY-1] == "." {
			// fmt.Println("going left")
